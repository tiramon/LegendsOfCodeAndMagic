...
public class Card {
	...
	public enum Type {
		CREATURE("creature"), ITEM_GREEN("itemGreen"), ITEM_RED("itemRed"), ITEM_BLUE("itemBlue");
	...
	// TODO maybe myHealthChange, oppHealthChange, cardDraw should be moved into
	// Summon class?
	...
	// data = {baseId, name, type, cost, attack, defense, keywords, myHealthChange,
	// oppHealthChange, cardDraw, comment}
	...
	private String toTextDescription() {
		...
		if (type == Type.CREATURE) {
			...
			if (attack != 0 || defense != 0)
				sb.append(" Give friendly creature ").append(nf.format(attack)).append(" / ")
						.append(nf.format(defense));
		} else if (type == Type.ITEM_RED) {
			sb.append(text);
			if (attack != 0 || defense != 0)
				sb.append(" Give enemy creature ").append(attack).append(" / ").append(defense);
		} else {
			sb.append(defense != 0 ? ("Deal " + Math.abs(defense) + " damage. ") : "")
					.append(text.length() > 1 ? text : "");
		}
	...
	public String toHTMLString() {
		...
		// String portraitpath = String.format("src=\"../assets/card_images/%03d.png\"",
		// baseId);
		String portraitimage = String.format("src=\"portraits/%03d.png\"", baseId);
		String cardimage = String.format("src=\"cards/%03d.png\"", baseId);
		// sb.append("<td>").append("<a><img "+portraitimage+" width=\"50\"><img
		// class=\"enlarge\" "+cardimage+" ></a>").append("</td>"); // width="300" --
		// deprecated for now
		sb.append("<td>")
				.append("<a><img " + portraitimage + " width=\"50\"><img class=\"enlarge\" " + portraitimage + " ></a>")
				.append("</td>"); // width="300" -- deprecated for now
		...
		while (m.find()) {
			...
			String link = String.format(
					"<a href=\"https://www.legends-decks.com/card/%s/%s\">%s<img class=\"enlarge\" src=\"https://www.legends-decks.com/img_cards/%s.png\"></a>",
